...
local function create_entity_replace(character, character_config, target_position, item, direction)

	local stack, stack_location = character.get_inventory(1).find_item_stack(item)
	if not stack or not stack.valid then
		Error("Trying to create an entity of "..item.." but couldn't find a stack of them in characters inventory")
		return false
	end
	...
	local created_entity = global.fle.game_surface.create_entity{name = item, position = target_position, direction = direction, force="player", fast_replace=true, character=character, raise_built = true, item = stack}
	...
	if created_entity and fast_replace_type_lookup[created_entity.name] ~= nil and created_entity.neighbours  then --connected entities eg underground belt https://lua-api.factorio.com/latest/LuaEntity.html#LuaEntity.neighbours
		...
		local entities_between = global.fle.game_surface.find_entities_filtered{name = replace_type, area = {
...
function build(character, character_config, target_position, item, direction)
	...
	if count < 1 then
		if(character_config.step_number > character_config.step_reached) then
			if character_config.walking.walking == false then
				Warning(string.format("Step_number: %d - Take: %s not available", character_config.step_number, fle_utils.format_name(item)))
	...
	if (item ~= "rail") then
		if fle_utils.item_is_tile(item) then
			if tile_is_in_reach() then
				if item == "stone-brick" then
					global.fle.game_surface.set_tiles({{position = target_position, name = "stone-path"}})
				elseif (item == "hazard-concrete") or (item == "refined-hazard-concrete") then
					global.fle.game_surface.set_tiles({{position = target_position, name = item.."-left"}})
				else
					global.fle.game_surface.set_tiles({{position = target_position, name = item}})
				end
                
                if(item == "landfill") then
                    global.fle.game_surface.play_sound{path="tile-build-small/landfill", position=target_position}
                else
                    global.fle.game_surface.play_sound{path="tile-build-small/concrete", position=target_position}
                end
		...
		elseif global.fle.game_surface.can_place_entity{name = item, position = target_position, direction = direction} then
			-- end_warning_mode(string.format("Step: %s, Action: %s, Step: %d - Build: [item=%s]", global.tas.task[1], global.tas.task[2], global.tas.step, item ))
			return create_entity_replace(character, character_config, target_position, item, direction)
		else
		...
		if global.fle.game_surface.can_place_entity{name = item, position = target_position, direction = direction} then
			
			if global.fle.game_surface.create_entity{name = item, position = target_position, direction = direction, force="player", raise_built = true} then
				character.remove_item({name = item, count = 1})
				-- end_warning_mode(string.format("Step: %s, Action: %s, Step: %d - Build: [item=%s]", global.tas.task[1], global.tas.task[2], global.tas.step, item ))
				return true
			end
